// =============================================
// Module 12: Claude Code Advanced - App JS
// =============================================
...
function getCompletedLessons() {
  const saved = localStorage.getItem('module12_completed');
  return saved ? JSON.parse(saved) : [];
}

function saveCompletedLessons(lessons) {
  localStorage.setItem('module12_completed', JSON.stringify(lessons));
}
...
function markLessonComplete(lessonNum) {
  ...
  if (!completed.includes(lessonNum)) {
    ...
    updateProgressUI(completed.length, 7);
...
function getCurrentLesson() {
  ...
  if (path.includes('project')) return 7;
  return 0;
}

function getChecklistState(lesson) {
  const saved = localStorage.getItem(`module12_checklist_${lesson}`);
  return saved ? JSON.parse(saved) : {};
}

function saveChecklistState(lesson, state) {
  localStorage.setItem(`module12_checklist_${lesson}`, JSON.stringify(state));
}
...
function resetProgress() {
  localStorage.removeItem('module12_completed');
  for (let i = 1; i <= 7; i++) {
    localStorage.removeItem(`module12_checklist_${i}`);
  }
  location.reload();
}

window.resetModule12Progress = resetProgress;
