 ...
 * Интерактивная презентация - Модуль 5
 */
...
// Ключ для хранения прогресса уроков внутри модуля 5
...
function getUnlockedLessons() {
  ...
  completed.forEach(id => {
    ...
    if (!unlocked.includes(num + 1) && num + 1 <= 9) unlocked.push(num + 1);
  });
  return unlocked;
}
...
function initLessonLocks() {
  ...
  lessonCards.forEach(card => {
    ...
    // Handle project.html (lesson 9)
    if (href === 'project.html' && !UNLOCKED_LESSONS.includes(9)) {
      card.classList.add('locked');
      card.addEventListener('click', (e) => {
        e.preventDefault();
        showLockedMessage(9);
  ...
  if (currentPath.includes('project.html') && !UNLOCKED_LESSONS.includes(9)) {
    window.location.href = 'index.html';
  }
}
...
function initTimer() {
  ...
  const savedTime = sessionStorage.getItem('lessonTimer_m5');
  if (savedTime) {
...
function startTimer(display) {
  ...
  timerInterval = setInterval(() => {
    ...
    sessionStorage.setItem('lessonTimer_m5', timerSeconds);
  }, 1000);
}
...
function resetTimer(display) {
  ...
  timerSeconds = 0;
  updateTimerDisplay(display);
  sessionStorage.removeItem('lessonTimer_m5');
  const timerBtn = document.querySelector('.timer-btn');
  if (timerBtn) {
    timerBtn.textContent = '▶';
    timerBtn.title = 'Запустить';
...
function initChecklists() {
  ...
  checkboxes.forEach(checkbox => {
    ...
    if (id) {
      const saved = localStorage.getItem(`checklist_m5_${id}`);
      if (saved === 'true') {
    ...
    checkbox.addEventListener('click', () => {
      ...
      // Save state
      if (id) {
        localStorage.setItem(`checklist_m5_${id}`, isChecked);
      }
...
function updateOverallProgress(completed) {
  const totalLessons = 9; // 8 lessons + project
