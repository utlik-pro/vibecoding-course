// =============================================
// Module 11: Claude Code Basics - App JS
// =============================================
...
function getCompletedLessons() {
  const saved = localStorage.getItem('module11_completed');
  return saved ? JSON.parse(saved) : [];
}

function saveCompletedLessons(lessons) {
  localStorage.setItem('module11_completed', JSON.stringify(lessons));
}
...
function markLessonComplete(lessonNum) {
  ...
  if (!completed.includes(lessonNum)) {
    ...
    updateProgressUI(completed.length, 6);
...
function getCurrentLesson() {
  ...
  if (path.includes('project')) return 6;
  return 0;
}

function getChecklistState(lesson) {
  const saved = localStorage.getItem(`module11_checklist_${lesson}`);
  return saved ? JSON.parse(saved) : {};
}

function saveChecklistState(lesson, state) {
  localStorage.setItem(`module11_checklist_${lesson}`, JSON.stringify(state));
}
...
function resetProgress() {
  localStorage.removeItem('module11_completed');
  for (let i = 1; i <= 6; i++) {
    localStorage.removeItem(`module11_checklist_${i}`);
  }
  location.reload();
}
