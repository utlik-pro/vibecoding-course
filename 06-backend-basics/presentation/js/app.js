 ...
 * Интерактивная презентация - Модуль 6
 */
...
// Ключ для хранения прогресса уроков внутри модуля 6
const MODULE_KEY = 'completedLessons_m6';
const MODULE_ID = 6;
const TOTAL_LESSONS = 10; // 9 уроков + project
...
function initTimer() {
  ...
  const savedTime = sessionStorage.getItem('lessonTimer_m6');
  if (savedTime) {
...
function startTimer(display) {
  ...
  timerInterval = setInterval(() => {
    ...
    sessionStorage.setItem('lessonTimer_m6', timerSeconds);
  }, 1000);
}
...
function resetTimer(display) {
  ...
  timerSeconds = 0;
  updateTimerDisplay(display);
  sessionStorage.removeItem('lessonTimer_m6');
  const timerBtn = document.querySelector('.timer-btn');
  if (timerBtn) {
    timerBtn.textContent = '▶';
    timerBtn.title = 'Запустить';
...
function initChecklists() {
  ...
  checkboxes.forEach(checkbox => {
    ...
    if (id) {
      const saved = localStorage.getItem(`checklist_m6_${id}`);
      if (saved === 'true') {
    ...
    checkbox.addEventListener('click', () => {
      ...
      // Save state
      if (id) {
        localStorage.setItem(`checklist_m6_${id}`, isChecked);
      }
...
function updateOverallProgress(completed) {
  const totalLessons = 10; // 9 lessons + project
