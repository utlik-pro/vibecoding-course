...
Модуль построен иначе, чем классический «теория → практика»:

1. В **уроке 6.1** студенты берут готовый PRD из файла [`PRD.md`](./PRD.md)
   в корне модуля и скармливают его `task-master parse-prd` (это инструмент
   из урока 5.4 предыдущего модуля).
2. Task Master выдаёт ~10 конкретных задач с подзадачами.
3. **Уроки 6.4–6.9** идут «по задачам»: на каждом уроке закрывается одна
   (или несколько связанных) задач из плана Task Master.
4. На каждом этапе разработки **мы останавливаемся** и объясняем теорию
   в контексте того, что только что сгенерировал Cursor/Claude Code.

**Почему так лучше для нулевиков:** теория, привязанная к моменту
«я только что увидел, как работает мой код», запоминается несравнимо
лучше, чем теория, оторванная от практики.

## Связка «урок ↔ задача из Task Master»

| Урок | Закрываем задачу(и) | Теория в контексте                |
| ---- | ------------------- | --------------------------------- |
| 6.1  | (запуск PRD)        | Бэкенд / фронтенд / БД на пальцах |
| 6.2  | —                   | API, HTTP-методы, JSON            |
| 6.3  | —                   | База данных, таблицы, типы        |
| 6.4  | 1                   | RLS, ключи Supabase, ssr-клиенты  |
| 6.5  | 2                   | Колонки, типы, первичный ключ     |
| 6.6  | 3, 8                | Server Component vs API Route     |
| 6.7  | 4, 5, 6, 7          | Server Actions, zod, revalidate   |
| 6.8  | 9                   | Тестирование, коды ошибок         |
| 6.9  | 10 + финальная UI   | Сборка всего в работающее SPA     |
