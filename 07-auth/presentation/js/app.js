 ...
 * Интерактивная презентация - Модуль 7
 */
...
// Ключ для хранения прогресса уроков внутри модуля 7
const MODULE_KEY = 'completedLessons_m7';
const MODULE_ID = 7;
const TOTAL_LESSONS = 9; // 8 уроков + project
...
function initTimer() {
  ...
  const savedTime = sessionStorage.getItem('lessonTimer_m7');
  if (savedTime) {
...
function startTimer(display) {
  ...
  timerInterval = setInterval(() => {
    ...
    sessionStorage.setItem('lessonTimer_m7', timerSeconds);
  }, 1000);
}
...
function resetTimer(display) {
  ...
  timerSeconds = 0;
  updateTimerDisplay(display);
  sessionStorage.removeItem('lessonTimer_m7');
  const timerBtn = document.querySelector('.timer-btn');
  if (timerBtn) {
    timerBtn.textContent = '▶';
    timerBtn.title = 'Запустить';
