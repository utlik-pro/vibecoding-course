 ...
 * Интерактивная презентация - Бонусный модуль "Как писать PRD для Task Master"
 */

// ==========================================
// LESSON ACCESS CONFIGURATION
// ==========================================
// Бонусный модуль — все 3 урока открыты с самого начала (нет порядка прохождения)
const MODULE_KEY = 'completedLessons_bonus_prd';
const MODULE_ID = 'bonus_prd';
const TOTAL_LESSONS = 3;

function getUnlockedLessons() {
  // Бонус всегда полностью открыт
  return [1, 2, 3];
}
...
function initLessonLocks() {
  ...
  lessonCards.forEach(card => {
    ...
    // Бонусный модуль — без project.html, ничего не лочим
  });

  // Бонусный модуль — все уроки всегда открыты
}
...
function initTimer() {
  ...
  const savedTime = sessionStorage.getItem('lessonTimer_bonus_prd');
  if (savedTime) {
...
function startTimer(display) {
  ...
  timerInterval = setInterval(() => {
    ...
    sessionStorage.setItem('lessonTimer_bonus_prd', timerSeconds);
  }, 1000);
}
...
function resetTimer(display) {
  ...
  timerSeconds = 0;
  updateTimerDisplay(display);
  sessionStorage.removeItem('lessonTimer_bonus_prd');
  const timerBtn = document.querySelector('.timer-btn');
  if (timerBtn) {
    timerBtn.textContent = '▶';
    timerBtn.title = 'Запустить';
...
function initChecklists() {
  ...
  checkboxes.forEach(checkbox => {
    ...
    if (id) {
      const saved = localStorage.getItem(`checklist_bonus_prd_${id}`);
      if (saved === 'true') {
    ...
    checkbox.addEventListener('click', () => {
      ...
      // Save state
      if (id) {
        localStorage.setItem(`checklist_bonus_prd_${id}`, isChecked);
      }
...
function updateOverallProgress(completed) {
  const totalLessons = 3; // бонусный модуль — 3 урока
